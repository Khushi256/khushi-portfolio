...
function Projects() {
    return (
        <section
            className="relative bg-primary dark:bg-background-light text-white dark:text-primary transition-colors duration-300 py-20 md:py-32 px-6"
            id="projects"
        ...
        >
            ...
            <div className="max-w-7xl mx-auto w-full mb-12 md:mb-20">
                ...
                <p className="text-base md:text-xl text-zinc-400 dark:text-zinc-600 max-w-lg mt-4 md:mt-6">
            ...
            <div className="max-w-7xl mx-auto w-full flex flex-col gap-16 md:gap-24">
                {projects.map((project, index) => (
                    <motion.div
                        ...
                        className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 items-center border-t border-zinc-800 dark:border-zinc-200 pt-10 md:pt-12"
                    >
                        {/* Text side */}
                        <div className="space-y-4 order-2 md:order-1 min-w-0">
                            <div className="overflow-hidden">
                                <span className="font-display text-[15vw] md:text-[6rem] lg:text-[7rem] font-extrabold leading-none text-transparent stroke-text text-yellow-500 opacity-70 dark:opacity-50 block">
                                    {project.number}
                                </span>
                            </div>
                            ...
                            <p className="text-base md:text-lg text-zinc-400 dark:text-zinc-600 max-w-md">
                                {project.description}
                            </p>
                            <div className="flex flex-wrap gap-2 md:gap-3">
                                {project.tags.map((tag, tagIndex) => (
                                    <span
                                        key={tagIndex}
                                        className="px-3 py-1.5 md:px-4 md:py-2 border border-zinc-700 dark:border-zinc-300 rounded-full text-xs md:text-sm font-bold uppercase tracking-widest hover:border-yellow-500 hover:text-yellow-500 transition-colors"
                                    >
                                        {tag}
                                    </span>
                                ))}
                            ...
                            <div className="flex flex-wrap gap-3 pt-2">
                                <a
                                    ...
                                    className="px-5 py-2 text-xs bg-white dark:bg-primary text-primary dark:text-white rounded-full font-bold uppercase tracking-widest hover:scale-105 hover:shadow-xl transition-all flex items-center gap-2"
                                >
                                    GitHub
                                ...
                                {project.live && (
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="px-5 py-2 text-xs border border-zinc-300 dark:border-zinc-700 text-white dark:text-primary rounded-full font-bold uppercase tracking-widest hover:scale-105 hover:bg-zinc-800 dark:hover:bg-zinc-100 transition-all flex items-center gap-2"
                                    >
                                        Live Demo
                        ...
                        <div className="h-[45vw] max-h-[320px] md:max-h-none md:h-[55vh] lg:h-[65vh] w-full flex items-center justify-center order-1 md:order-2">
                            <img
                                src={project.image}
                                alt={project.title}
                                className="max-w-full max-h-full object-contain rounded-2xl md:rounded-3xl shadow-[0_0_40px_rgba(255,255,255,0.3)] dark:shadow-[0_0_40px_rgba(0,0,0,0.3)]"
                            />
                        </div>
                    </motion.div>
                ))}
